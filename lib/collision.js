  ...
  for (let i = 0; i < hitbox1.lines.length; i++) {
    for (let j = 0; j < hitbox2.lines.length; j++) {
      ...
      if (intersects.t >= 0 && intersects.t <= 1 && intersects.u >= 0 && intersects.u <= 1) {
        let dy1 = hitbox1.lines[i][0].y - hitbox1.lines[i][1].y;
        let dx1 = hitbox1.lines[i][0].x - hitbox1.lines[i][1].x;
        let slope1 = dy1 / dx1;

        let dy2 = hitbox2.lines[j][0].y - hitbox2.lines[j][1].y;
        let dx2 = hitbox2.lines[j][0].x - hitbox2.lines[j][1].x;
        let slope2 = dy2 / dx2;
        ...
        if (!right && i == 1 && hitbox1.pright == hitbox2.pleft && hitbox1.pbottom > hitbox2.ptop && hitbox1.ptop < hitbox2.pbottom) {
          right = true;

        };
        if (!left && i == 3 && hitbox1.pleft == hitbox2.pright && hitbox1.pbottom > hitbox2.ptop && hitbox1.ptop < hitbox2.pbottom) {
          left = true;
          //console.log(hitbox2)
        };
